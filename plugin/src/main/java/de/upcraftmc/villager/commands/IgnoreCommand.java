...
/**
 * AntiVillagerLag; de.upcraftmc.villager.commands:IgnoreCommand
 *
 * @author <a href="https://github.com/LuciferMorningstarDev">LuciferMorningstarDev</a>
 * @since 19.03.2023
 */
public class IgnoreCommand implements AVLSubCommand {

    private UpCraftAntiVillagerLagPlugin plugin;

    public IgnoreCommand(UpCraftAntiVillagerLagPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean run(final Player sender, final List subArgs) {
        ...
        boolean mod = VillagerUtiils.ignore(v, plugin);
        if (mod) {
            VillagerUtiils.setIgnore(v, plugin, false);
        } else {
            VillagerUtiils.setIgnore(v, plugin, true);
        }

        String done = mod ? "&causgeschaltet" : "&aeingeschaltet";
        sender.sendMessage(ComponentSerializer.etAndHEX.deserialize("&7Der Villager UmgebungsCheck wurde " + done + "&7."));
