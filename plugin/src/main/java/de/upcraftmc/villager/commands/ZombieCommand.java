...
/**
 * AntiVillagerLag; de.upcraftmc.villager.commands:ZombieCommand
 *
 * @author <a href="https://github.com/LuciferMorningstarDev">LuciferMorningstarDev</a>
 * @since 19.03.2023
 */
public class ZombieCommand implements AVLSubCommand {

    private UpCraftAntiVillagerLagPlugin plugin;

    public ZombieCommand(UpCraftAntiVillagerLagPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean run(final Player sender, final List subArgs) {
        ...
        if (!VillagerUtiils.shouldBeDisabled(v, plugin)) {
            sender.sendMessage(ComponentSerializer.etAndHEX.deserialize("&cNur deaktivierte Villager können den Zombieschutz genießen."));
            return false;
        }

        boolean prot = VillagerUtiils.isProtected(v, plugin);

        if (VillagerUtiils.isProtected(v, plugin)) {
            VillagerUtiils.removeProtected(v, plugin);
        } else {
            VillagerUtiils.addProtected(v, plugin);
        }

        String done = prot ? "&aeingeschaltet" : "&causgeschaltet";
        sender.sendMessage(ComponentSerializer.etAndHEX.deserialize("&7Der Villager Zombieschutz wurde " + done + "&7."));
